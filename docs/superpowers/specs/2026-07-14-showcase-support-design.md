...
| D2 | **Reuse the incident downstream by classifying showcases as `display_type=9`.** No new tracking type. | `incident` template, `ResolveGruntTypeName→"showcase"`, v2 `/incident` tracking, and `translateShowcaseRankings` already handle `display_type=9`. |
