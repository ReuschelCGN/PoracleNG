...
- [ ] **Resource helpers** — human-scoped addressing (`{id}` path), `(human, uid)` ownership scoping on item ops, `profile`/`include_descriptions`/`silent` query binding; list → `{rules:[…]}`; create → `{created,updated,unchanged, message}` with uids. **Mutation responses return the assembled `message`** (human-readable added/updated/removed summary, human's language) — always, independent of `silent` (which only gates the push); NO per-rule `description` on mutations (rowtext lives once, in `message`). Per-rule `description` is reads-only via `?include_descriptions`. Reuse the existing rowtext generator + `translatorFor`.
